...
```{r}
 tokenized_corpus %>%
  count(word, sort = TRUE) %>%
    ...
    ggplot(aes(word, n)) +
    geom_col() +
  labs(title = "Most Common Words Included in Episode Descriptions", subtitle = "*Pruned vocab not included") +
    xlab(NULL) +
...
corpus_lda2 <- LDA(corpus_dtm, k = 10, control = list(seed = 6432))
